...
function ThemeToggle() {
  ...
  return (
    ...
    >
      ...
      <MoonIcon className="hidden h-6 w-6 fill-stone-700 stroke-stone-500 transition dark:block [@media(prefers-color-scheme:dark)]:group-hover:stroke-stone-400 [@media_not_(prefers-color-scheme:dark)]:fill-amber-400/10 [@media_not_(prefers-color-scheme:dark)]:stroke-amber-500" />
    </button>
  )
}
  ...
  useEffect(() => {
    ...
    function updateAvatarStyles() {
      ...
      setProperty(
        ...
        `transtone3d(${x}rem, 0, 0) scale(${scale})`,
      )

      let borderScale = 1 / (toScale / scale)
      let borderX = (-toX + x) * borderScale
      let borderTransform = `transtone3d(${borderX}rem, 0, 0) scale(${borderScale})`
